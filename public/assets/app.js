...
const ball=(n,sm=false)=>`<span class="ball ${sm?"sm ":""}${ballClass(n)}">${n}</span>`;
const dateKo=s=>{const d=new Date(`${s}T00:00:00`);return `${d.getFullYear()}년 ${d.getMonth()+1}월 ${d.getDate()}일 추첨`};
const money=n=>n?`${Number(n).toLocaleString("ko-KR")}원`:"공식 결과 확인";
function toast(msg){const e=$("#toast");e.textContent=msg;e.classList.add("show");setTimeout(()=>e.classList.remove("show"),1700)}
function nextDrawText(){
 const now=new Date(),next=new Date(now);let add=(6-now.getDay()+7)%7;
 if(add===0&&(now.getHours()>20||(now.getHours()===20&&now.getMinutes()>=35)))add=7;
 next.setDate(now.getDate()+add);next.setHours(20,35,0,0);
 const diff=next-now,h=Math.max(0,Math.floor(diff/3600000)),d=Math.floor(h/24);
 return d>0?`D-${d}`:(h>0?`${h}시간 후`:"오늘 추첨")
}
function renderDraws(draws,live){
 ...
 $("#winnerCount").textContent=d.winners?`${d.winners}명`:"공식 결과 확인";$("#firstPrize").textContent=money(d.prize);$("#nextDraw").textContent=nextDrawText();
 const st=$("#status");st.textContent=live?"● 최신 반영":"● 저장 데이터";st.classList.toggle("fallback",!live);
 $("#historyList").innerHTML=draws.slice(0,5).map(x=>`<div class="history-row"><div class="row-meta"><strong>제${x.round}회</strong><span>${x.date.replaceAll("-",".")}</span></div><div class="balls">${x.numbers.map(n=>ball(n,true)).join("")}<span class="plus">+</span>${ball(x.bonus,true)}</div></div>`).join("");
 renderFrequency(draws);
}
async function loadDraws(){
 $("#historyList").innerHTML='<div class="history-row"><span style="color:var(--muted);font-size:12px">최근 당첨정보를 불러오는 중입니다.</span></div>';
 ...
 catch{renderDraws(FALLBACK,false)}
}
...
function valid(a){
 ...
 if(state.noConsecutive&&a.some((n,i)=>i&&n-a[i-1]===1))return false;
 return true
}
function makeOne(){
 const pool=Array.from({length:45},(_,i)=>i+1).filter(n=>!excluded().includes(n));if(pool.length<6)throw Error("제외 번호가 너무 많습니다.");
 for(let i=0;i<4000;i++){const a=shuffle(pool).slice(0,6).sort((x,y)=>x-y);if(valid(a))return a}
 return shuffle(pool).slice(0,6).sort((x,y)=>x-y)
}
function generate(){
 try{
...
function saveNumbers(nums){
 const list=saved(),key=nums.join("-");
 if(list.some(x=>x.join("-")===key))return toast("이미 저장된 번호입니다.");
 list.unshift(nums);localStorage.setItem("luckySaved",JSON.stringify(list.slice(0,20)));renderSaved();toast("번호를 저장했습니다.")
}
function renderSaved(){
 const list=saved();$("#savedList").innerHTML=list.length?list.map((x,i)=>`<div class="saved-row"><strong>${x.join(" · ")}</strong><button class="delete" data-i="${i}">삭제</button></div>`).join(""):`<p style="color:var(--muted);font-size:13px">저장된 번호가 없습니다.</p>`;
 $$(".delete").forEach(b=>b.onclick=()=>{const l=saved();l.splice(+b.dataset.i,1);localStorage.setItem("luckySaved",JSON.stringify(l));renderSaved()})
}
function renderFrequency(draws){
 const counts=Array(46).fill(0);draws.forEach(d=>d.numbers.forEach(n=>counts[n]++));
 const top=Array.from({length:45},(_,i)=>({n:i+1,c:counts[i+1]})).sort((a,b)=>b.c-a.c||a.n-b.n).slice(0,10),max=Math.max(...top.map(x=>x.c),1);
 $("#frequency").innerHTML=top.map(x=>`<div class="freq"><strong>${x.n}</strong><span class="bar"><i style="width:${x.c/max*100}%"></i></span><span>${x.c}회</span></div>`).join("")
}
async function fetchRound(round){
 const local=state.draws.find(x=>x.round===round);if(local)return local;
 for(let i=0;i<3;i++){try{const r=await fetch(`api/lotto?round=${round}&retry=${i}`,{cache:"no-store"});if(r.ok){const j=await r.json();if(j.draws?.[0])return j.draws[0]}}catch{}await new Promise(r=>setTimeout(r,250*(i+1)))}return null
}
$("#roundButton").onclick=async()=>{
 const round=+$("#roundInput").value;if(!Number.isInteger(round)||round<1)return toast("올바른 회차를 입력하세요.");
 $("#roundButton").disabled=true;$("#roundButton").textContent="조회 중";
 const d=await fetchRound(round);$("#roundButton").disabled=false;$("#roundButton").textContent="검색";
 $("#roundResult").innerHTML=d?`<div class="history-row"><div class="row-meta"><strong>제${d.round}회</strong><span>${d.date}</span></div><div class="balls">${d.numbers.map(n=>ball(n)).join("")}<span class="plus">+</span>${ball(d.bonus)}</div><p style="color:var(--muted);font-size:12px;margin:11px 0 0">1등 당첨자 ${d.winners||"확인 필요"} · 1게임당 ${money(d.prize)}</p></div>`:`<div class="history-row"><strong>제${round}회 정보를 불러오지 못했습니다.</strong><p style="color:var(--muted);font-size:12px">잠시 후 다시 조회하거나 공식 결과를 확인하세요.</p></div>`
};
function renderToday(force=false){
 const date=new Date().toISOString().slice(0,10);let data=JSON.parse(localStorage.getItem("todayPick")||"null");
 if(force||!data||data.date!==date){data={date,nums:shuffle(Array.from({length:45},(_,i)=>i+1)).slice(0,6).sort((a,b)=>a-b)};localStorage.setItem("todayPick",JSON.stringify(data))}
 $("#todayBalls").innerHTML=data.nums.map(n=>ball(n)).join("")
}
...
$("#roundInput").addEventListener("keydown",e=>{if(e.key==="Enter")$("#roundButton").click()});
